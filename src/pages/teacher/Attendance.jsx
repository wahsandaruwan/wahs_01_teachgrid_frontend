...
const Attendance = () => {
  ...
  const filteredData = attendanceData.filter((row) => row.date.includes(search))

  return (
    <main className="flex-1 p-10">
      <h1 className="text-2xl mb-6">Attendance Records</h1>
      ...
      <div className="bg-white rounded-xl shadow-md p-8">
        <div className="flex flex-col gap-4 items-start justify-between md:flex-row md:items-center mb-6">
          <div>
            ...
            <div className="text-[#909cb5] text-sm">View your personal attendance history and download reports</div>
          </div>
          <div className="flex gap-3">
            ...
            />
            <button className="bg-[#263066] text-white border-none px-4 py-2 rounded cursor-pointer">Export</button>
          </div>
        </div>
        ...
        <table className="w-full mt-1 border-collapse">
          ...
          <tbody>
            {filteredData.map((row, index) => (
              <tr key={row.date} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fafbfc]'}>
...
const StatCard = ({ title, value, subtitle }) => {
  return (
    <div className="bg-white rounded-xl p-7 shadow-md flex-1 min-w-[250px]">
